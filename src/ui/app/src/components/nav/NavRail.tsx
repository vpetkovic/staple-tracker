 ...
 *
 * The rail draws whatever `NAV_GROUPS` says. It does not know the names of the views.
 */
import { ChevronDown, Moon, PanelLeftClose, Plus, Search, Settings, Sun } from "lucide-react";
...
import { NAV_GROUPS, type NavGroup } from "./nav-model";
...
export const RAIL_ROW_CLASS = cn(
  ...
  "aria-[current=page]:bg-surface-active aria-[current=page]:font-medium aria-[current=page]:text-foreground",
);
    ...
    >
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-2 pt-1">
        {NAV_GROUPS.map((group) => (
          <NavGroupSection key={group.id} group={group} view={session.view} onSelect={select} />
        ))}
      </div>
