 ...
 *     derived from the old `repoId`, so they cannot be re-aimed. Nothing replays
 *     them: the new repository has received nothing, so the first sync SEEDS it
 *     with the workspace's whole current state (`cloud/seed.ts`, which keys its
 *     once-only record by repository id for exactly this case).
