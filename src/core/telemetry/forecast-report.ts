...
export const FORECAST_METHOD = {
  ...
  band: BAND,
  remaining: {
    expected: "max(0, calibrated expected duration - workSeconds)",
    draws: "a class sample ratio longer than the work so far, uniformly, times the estimate, less the work",
    done: "0",
    awaitingReview: "0: review and approval waits are not work; rework after review is not forecast",
  ...
  budget: {
    ...
    workRate: "the burn of this workspace's worker attempts that started in the current window, per hour of their effortSeconds; bootstrap-resampled",
    rateAttemptLimit: RATE_ATTEMPT_LIMIT,
    schedule: "the work runs serially from asOf, one work-hour per hour",
    breach: "the share of draws (labor draw x work-rate draw) that leave less than the reserve at the reset",
  ...
  caveats: [
    ...
    "the work rate counts other use of the account during an attempt when the attempt was not alone on it (shared)",
