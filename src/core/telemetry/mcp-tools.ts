...
export function registerBudgetTools(server: McpServer, helpers: { run: Run }): void {
  server.registerTool(
    ...
    {
      description:
        "Record provider usage readings on THIS machine (docs/execution-telemetry.md). source='claude-statusline' takes the status-line JSON Claude Code sent (input) and binds it by config_dir (default CLAUDE_CONFIG_DIR, then ~/.claude); 'codex-rollout' reads one rollout file (file), skipping fork-copied history; 'manual' records a reading an operator read off /usage or /status (account, limit_key, used, optional resets_at as an instant or a duration like 3h, optional provider). Harness sources need budget capture switched on (`staple budget capture on`, off by default), and every source needs an account: a machine binding (`staple budget bind`) or account. Without one the call is refused and nothing is stored under a guessed account. Percentages are stored as reported, never inferred; an unknown value is null with a reason in `missing`, never 0. Returns one outcome per reading: the stored sample, or {stored:false, reason} with reason unchanged, fork_copied, not_reported_by_source or parse_error. Samples stay in this machine's hub.db and never replicate; no network request is made.",
      inputSchema: {
