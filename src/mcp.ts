...
server.registerTool(
  ...
  {
    description:
      "How much of a population's timing can be trusted, for analytics filtering (docs/timing-semantics.md, \"Quality states\"). Every timing record has exactly one quality state with the reasons that produced it: work (the estimate ratio's actual) is exact, timing-floor (under 60 s: quick, and little more), approximate (sparse, capture_gap, contested, partial, ...), reconstructed (backfilled before capture) or missing; wall is exact, approximate or missing. Eligible population, the denominator of every coverage figure: the leaves resolved done in the filter. Returns counts and coverage per state, how many records carry each reason, the estimate ratio over done issues with their own estimate and no estimated descendant (exact records only, and over the admitted states), and the eligible records themselves, bounded and cursored. include names the states kept (default all) and exclude the states dropped; a record is kept only when its state and the level of every reason it carries are kept, so exclude [\"approximate\"] also drops a reconstructed record that is sparse, and include [\"exact\"] is exact records only. exclude_reasons drops records carrying a reason code whatever their state (e.g. [\"sparse\"]). Dropped records leave items and the admitted ratio; counts and coverage never change, and nothing is dropped by default, so timing-floor records stay listed. Same payload as `staple timing quality --json`.",
    inputSchema: {
      ...
      include: z.array(z.string()).optional().describe("Work states the analysis keeps (default all): a record is kept only when its state and the level of every reason it carries are kept. [\"exact\"] is exact records only; [\"exact\", \"reconstructed\"] adds reconstructed records with nothing approximate, missing or under a minute about them"),
      exclude: z.array(z.string()).optional().describe("Work states to drop from the analysis: every record whose state or any reason is at that level (exclude approximate also drops a reconstructed record that is sparse). missing, reconstructed, approximate, timing-floor, exact"),
  ...
  ({ kind, parent, since, include, exclude, exclude_reasons, limit, cursor, ws }) =>
    run(() => storeFor(ws).timingQuality({ kind, parent, since, include, exclude, excludeReasons: exclude_reasons, limit, cursor })),
);
...
server.registerTool(
  "calibration_cohorts",
  {
    ...
    inputSchema: {
      ...
      cursor: z.string().optional().describe("Opaque cursor from the previous page's nextCursor, with the same other arguments."),
      for: z.array(z.string()).optional().describe("Issues (identifier or id) to forecast a duration for: forecasts lists one per issue and evidence set. An issue with no worker attempt yet matches any model"),
      model: z.string().optional().describe("With for: the model the work will run on, pinned in each forecast's key (as --model on checkout names it)"),
      ws: wsSchema,
    },
    outputSchema: {
      asOf: z.string(),
  ...
  ({ kind, priority, parent, since, include, list, limit, cursor, for: forecast, model, ws }) =>
    run(() => storeFor(ws).calibration({ kind, priority, parent, since, include, list, limit, cursor, for: forecast, model })),
);
...
server.registerTool(
  ...
  {
    description:
      "Forecast an issue's completion and, separately, what that work costs this machine's provider budget (docs/timing-semantics.md, \"Forecasts\"). completion: the certified plan's units beneath ref (a leaf is its own unit), each forecast from its calibrated duration (the exact set, the class calibration_cohorts for [ref] reads) less the work already done: done units weigh 0, units in review or gated weigh 0 and are listed (review waits and rework are not forecast), a unit with no samples, no estimate or worked past every sample of its class is unknown, never 0, and turns sums partial. labor is the remaining work added; path is the longest dependency chain of remaining work (effort along the chain, not calendar time). expectedSeconds adds each unit's expected remainder (the calibrated figure when unworked, the mean of the admissible remainders once work has started); simulated is from resampling each unit's class sample ratios (conditioned on the unit not being done) with a fixed seed (method.seed, method.draws), so the same data reads the same everywhere: mean, p10/p50/p90 and band (p5-p95, nominal 0.9). confidence.achieved is the lowest prediction-bounds confidence of the classes drawn from; label high/medium/low with reasons; warnings carry the calibration warnings and unknown_units, beyond_class_range, overrun, few_admissible, awaiting_review, independent_draws, dependency_cycle, unresolved_outside_blockers. budget (machine-local, never blended with completion): per account and limit, the current window's high-water remainingPercent, resetsAt, pace (%/hour of wall clock over the window's readings), exhaustion at that pace before or after the reset, workRate (%/work-hour: the window's rise over the union of this workspace's worker attempt spans, per hour of their work, so concurrent attempts count once, spans with another session's readings or a reading naming no session (shared_use) left out when unshared ones exist, sparse edge readings flagged; its own bootstrap band and confidence), otherUse (%/hour outside those spans, from at least 30 minutes and 2 readings there, with a confidence; withOtherUse adds it only for the hours the work is not running), and for the remaining labor run serially from now through the reset and the windows after it: consumedPercent, beforeResetPercent, remainingAtResetPercent, outlastsResetProbability, exhaustionProbability, and reserve.breachProbability (the work alone, any window it runs in), currentWindowBreachProbability and withOtherUse. completion.settled is false while a unit is in review; completion.review lists those units. reserve is a parameter; without it a provisional default applies and says so (the admission policy will define it). A stale, elapsed or sliding window, no attempt burn or unknown labor reads null with the reason in missing (input_missing lists missingInputs), never 0. snapshot.id identifies the completion inputs (the same on every device, over calibration snapshot.id); snapshot.budget.id this machine's budget data. Same payload as `staple forecast <ref> --json`.",
    inputSchema: {
      ref: refSchema,
