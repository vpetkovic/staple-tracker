...
const SCENARIOS: readonly Scenario[] = [
  ...
  // A built-in removed and added back is not a built-in on any device — a fresh one holds
  // the built-in from its migrations, and learns it from the create.
  { method: "WorkspaceStore.removeStatus", name: "remove a built-in status", run: (w) => void w.a.store.removeStatus("in_review", { migrateTo: "todo" }, "alice") },
  { method: "WorkspaceStore.addStatus", name: "add a removed built-in status back", run: (w) => void w.a.store.addStatus({ id: "in_review", category: "review", label: "In review" }, "alice") },
  { method: "WorkspaceStore.removeKind", name: "remove a built-in kind", run: (w) => void w.a.store.removeKind("spike", { migrateTo: "task" }, "alice") },
  { method: "WorkspaceStore.addKind", name: "add a removed built-in kind back", run: (w) => void w.a.store.addKind({ id: "spike", label: "Spike" }, "alice") },
  {
