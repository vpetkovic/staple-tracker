...
import { pullOperations } from "../src/core/cloud/client.js";
...
describe("an epoch is a discontinuity, and it is never a silent reset", () => {
  ...
  /**
   * The same deadlock one layer down, and the one the guard above does NOT
   * cover.
   *
   * A device killed between snapshot pages keeps `bootstrap_cursor`, and that
   * cursor is epoch-scoped: `worker/src/cursor.ts` fences every cursor with
   * `assertCursorScope`, so replaying one from a superseded epoch is
   * `epoch_changed` from `GET /snapshot`. The resume runs BEFORE the drain, so
   * the handler that exists for a superseded *incremental* cursor never sees it.
   * The sync aborts, `bootstrap_cursor` is still there, and the next run replays
   * the same dead cursor. For ever.
   *
   * It is worse than the queued-work case because it also blocks the push: a
   * device in this state cannot complete a sync at all, so nothing it does
   * locally ever reaches the other machines, and the only symptom is a message
   * about a cursor that a human has no way to edit.
   *
   * The service here answers the way the DEPLOYED Worker does — `currentEpoch`
   * and no `epoch`, per `SyncError("epoch_changed", …, { currentEpoch })` — so
   * this pins the recovery against what is actually running rather than against
  ...
  it("recovers when the epoch moves under an INTERRUPTED bootstrap", async () => {
    ...
    // server rather than by arithmetic.
    b.store.createIssue({ title: "Queued on B across a restore" });
    server.bumpEpoch();
    server.bumpEpoch();

    const workerShaped: typeof fetch = async (input, init) => {
      const response = await server.fetch(input, init);
      if (response.ok) return response;
      const body = (await response.json()) as Record<string, unknown>;
      if (body.code === "epoch_changed") delete body.epoch;
    ...
    expect(report.pending, "B has nothing left queued").toBe(0);
    await a.sync({ fetchImpl: workerShaped });
    expect(titles(a.store)).toContain("Queued on B across a restore");

    // A second run is a clean no-op, which is what distinguishes a recovery
    // from a failure that happened to leave the database further along.
    const again = await b.sync({ fetchImpl: workerShaped });
    expect(again.bootstrap, "a recovered device does not re-bootstrap").toBeNull();
    expect(again.pulled.operations, "and there is nothing left to pull").toBe(0);
  });

  /**
   * The recovery above corrects a wrong epoch guess from the snapshot it then
   * takes, so it survives without this — which is exactly why the divergence
   * needs its own test rather than being left to the integration path.
   *
   * Every `epoch_changed` the deployed Worker raises carries `currentEpoch` and
   * none carries `epoch` (`worker/src/cursor.ts`, `worker/src/push.ts`). A client
   * that reads only `epoch` learns nothing from the service and recovers by
   * assuming exactly one restore happened.
   */
  it("reads the epoch off the wire under the name the Worker actually sends", async () => {
    const a = device("device-a", "token-a");
    a.store.createIssue({ title: "One" });
    await a.sync();
    const b = device("device-b", "token-b");
    ...
    const refused = await pullOperations(
      parseEndpoint(ENDPOINT),
      { repositoryId: REPO_ID, token: "token-b", deviceId: "device-b", cursor, limit: 10 },
      { fetchImpl: workerShaped },
    ).then(
      () => null,
      (error: unknown) => error as StapleError,
    );

    expect(refused, "a superseded cursor is refused").toBeInstanceOf(StapleError);
    expect(refused!.detail?.cloudCode).toBe("epoch_changed");
    expect(
      refused!.detail?.epoch ?? refused!.detail?.currentEpoch,
      "and the refusal says which epoch the service is on",
    ).toBe(3);
  });
});
