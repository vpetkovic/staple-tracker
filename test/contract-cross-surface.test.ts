...
beforeAll(async () => {
  ...
  expect((await cli("doc", "CON-1", "plan", "--put", planFile, "--ws", WS)).status).toBe(0);
  ...
  return [
    {
      ...
      cli: async () => await cli("start", "CON-1", "--agent", INTRUDER, "--ws", WS, "--json"),
    ...
    },
    {
      ...
      cli: async () => await cli("show", "CON-999", "--ws", WS, "--json"),
    ...
    },
    {
      ...
      cli: async () => await cli("doc", "CON-1", "plan", "--put", planFile, "--base", "99", "--ws", WS, "--json"),
    ...
    },
    {
      ...
      cli: async () => await cli("new", "Contract root task", "--ws", WS, "--json"),
...
describe("surface-specific framing around a shared triple", () => {
  it("each surface wraps the SAME envelope in its own transport signal", async () => {
    ...
    const cliResult = await cli("start", "CON-1", "--agent", INTRUDER, "--ws", WS, "--json");
  ...
  it("retryable is a property of the error, not of the surface", async () => {
    ...
    const cliEnv = cliEnvelope(
      await cli("doc", "CON-1", "plan", "--put", planFile, "--base", "99", "--ws", WS, "--json"),
    );
