...
describe.each([
  ...
  it("is detected as version 1 with migrations 2 through 10 pending", () => {
    withFixture(fixture, (path) => {
      const db = new DatabaseSync(path);
      try {
        const state = describeSchema(db, WORKSPACE_TARGET);
        expect(state.current).toBe(1);
        expect(state.latest).toBe(10);
        ...
        expect(state.pending).toEqual([2, 3, 4, 5, 6, 7, 8, 9, 10]);
      } finally {
        db.close();
      }
    });
  ...
  it("is stamped '10' as TEXT, the representation an old binary can still read", () => {
    withFixture(fixture, (path) => {
      const db = openDb(path);
      try {
        migrateWorkspace(db);
        ...
        expect(row).toEqual({ t: "text", value: "10" });
      } finally {
        db.close();
      }
    });
...
describe("a v2 workspace — the last shape before estimates", () => {
  it("is detected as version 2 with migrations 3 through 10 pending", () => {
  ...
  it("takes the estimate migration without its old column order being disturbed", () => {
    withFixture(FIXTURES.workspaceV2LegacyDdl, (path) => {
      ...
      try {
        ...
        expect(describeSchema(db, WORKSPACE_TARGET).pending).toEqual([3, 4, 5, 6, 7, 8, 9, 10]);
  ...
  it("the milestone migration preserves every issue and every configured kind and creates no rows", () => {
    withFixture(FIXTURES.workspaceV2, (path) => {
      ...
      try {
        ...
        expect(describeSchema(db, WORKSPACE_TARGET)).toMatchObject({ current: 6, pending: [7, 8, 9, 10] });
        const store = new WorkspaceStore(db, "legacyrepo", "LEG");
  ...
  it("the queue migration preserves every issue and leaves the queue empty", () => {
    withFixture(FIXTURES.workspaceV2, (path) => {
      ...
      try {
        ...
        expect(describeSchema(db, WORKSPACE_TARGET)).toMatchObject({ current: 7, pending: [8, 9, 10] });
        const store = new WorkspaceStore(db, "legacyrepo", "LEG");
        const issuesBefore = db.prepare("SELECT * FROM issues ORDER BY identifier").all();
        ...
        expect(db.prepare("SELECT key, value FROM meta ORDER BY key").all()).toEqual(
          metaBefore.map((row) =>
            (row as { key: string }).key === "schema_version" ? { key: "schema_version", value: "10" } : row,
          ),
        );
  ...
  it("the projects migration preserves every issue, files none of them, and creates no project", () => {
    withFixture(FIXTURES.workspaceV2, (path) => {
      ...
      try {
        ...
        expect(describeSchema(db, WORKSPACE_TARGET)).toMatchObject({ current: 8, pending: [9, 10] });
        const store = new WorkspaceStore(db, "legacyrepo", "LEG");
        const issuesBefore = db.prepare("SELECT * FROM issues ORDER BY identifier").all();
        const commentsBefore = store.listComments("LEG-1").length;
