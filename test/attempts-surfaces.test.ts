...
describe("staple attempt and the attempt flags", () => {
  it("open, pause, resume, checkpoint, interrupt, resume by re-claim, fail, and complete an attempt", async () => {
    const issue = (await cli("new", "Surface work", "--estimate", "1h")).json;
    const ref = String(issue.identifier);

    const claimed = await cli("checkout", ref, "--harness", "claude_code", "--harness-session", "s-1", "--model", "claude-x", "--attempt-key", "k-1");
    ...
    expect((await cli("attempt", "pause", ref, "--reason", "awaiting_reset")).json).toMatchObject({ id: first, state: "paused" });
    expect((await cli("attempt", "resume", ref)).json).toMatchObject({ id: first, state: "running" });
    expect((await cli("attempt", "milestone", ref, "-m", "half done")).json).toMatchObject({ id: first, state: "running" });
    expect((await cli("attempt", "interrupt", ref, "--reason", "provider_limit")).json).toMatchObject({
      id: first,
      state: "ended",
      outcome: "interrupted",
    ...
    const again = await cli("checkout", ref);
    expect(attemptOf(again.json)).toMatchObject({ openedBy: "reclaim", resumesAttemptId: first, ordinal: 2 });

    const failed = await cli("release", ref, "--outcome", "failed", "--reason", "cannot reproduce");
    ...
    const started = await cli("status", ref, "in_progress");
    expect(attemptOf(started.json)).toMatchObject({ openedBy: "status", claim: { scope: "none" }, agent: "agent-cli" });
    const done = await cli("done", ref);
    expect(done.json).toMatchObject({ status: "done" });
    expect(attemptOf(done.json)).toMatchObject({ outcome: "completed", endReason: "done" });
  }, 120_000);

  it("refuses with the existing envelope and exit codes", async () => {
    const ref = String((await cli("new", "Refusals")).json.identifier);
    const none = await cli("attempt", "pause", ref, "--reason", "operator");
    expect(none.status).toBe(4);
    expect(JSON.parse(none.stderr.trim())).toMatchObject({ code: "conflict" });
    await cli("checkout", ref);
    const inferred = await cli("attempt", "interrupt", ref, "--reason", "claim_stolen");
    expect(inferred.status).toBe(2);
    expect(JSON.parse(inferred.stderr.trim())).toMatchObject({ code: "validation" });
    const noSession = await cli("release", ref, "--outcome", "failed");
    expect(noSession.status).toBe(2);
  }, 60_000);

  it("reconstructs history from events, once", async () => {
    const first = await cli("attempt", "reconstruct");
    expect(first.status).toBe(0);
    expect(first.json).toMatchObject({ reconstructed: expect.any(Number), alreadyPresent: expect.any(Number) });
    expect((await cli("attempt", "reconstruct")).json).toMatchObject({ reconstructed: 0 });
  }, 60_000);
});
...
describe("history before capture, at the boundary", () => {
   ...
   */
  async function claimedByAnOlderBuild(title: string, agent: string): Promise<string> {
    const ref = String((await cli("new", title)).json.identifier);
    expect((await cli("checkout", ref, "--agent", agent)).status).toBe(0);
  ...
  it("a steal after the upgrade: the older build's tenure was interrupted, and a later command writes nothing over it", async () => {
    const ref = await claimedByAnOlderBuild("Stolen after the upgrade", "agent-old");
    expect((await cli("checkout", ref, "--agent", "agent-new", "--steal-if-stale", "0s")).status).toBe(0);
    expect((await cli("attempt", "reconstruct")).json).toMatchObject({ reconstructed: 1 });
    expect((await cli("comment", ref, "a command after")).status).toBe(0);
  ...
  it("the holder re-claiming after the upgrade: the tenure continued, and no interruption is ever written", async () => {
    const ref = await claimedByAnOlderBuild("Re-claimed after the upgrade", "agent-old");
    expect((await cli("checkout", ref, "--agent", "agent-old")).status).toBe(0);
    expect((await cli("attempt", "reconstruct")).json).toMatchObject({ reconstructed: 1 });
    expect((await cli("comment", ref, "a command after")).status).toBe(0);
    expect(rows(ref)).toEqual([
...
describe("MCP record_attempt_event", () => {
  it("answers the same shape as the CLI, from the same store method", async () => {
    const viaCli = String((await cli("new", "CLI milestone")).json.identifier);
    const viaMcp = String((await cli("new", "MCP milestone")).json.identifier);
    await cli("checkout", viaCli, "--agent", "agent-mcp");
    await cli("checkout", viaMcp, "--agent", "agent-mcp");

    const fromCli = (await cli("attempt", "milestone", viaCli, "-m", "checkpoint", "--agent", "agent-mcp")).json;
    ...
    const third = String((await cli("new", "MCP claim")).json.identifier);
...
describe("the orchestrator lane on the CLI and MCP", () => {
  it("opens and ends by attempt open|end --role orchestrator, and every claim write refuses a role", async () => {
    const epic = String((await cli("new", "Coordinated epic")).json.identifier);
    const opened = await cli("attempt", "open", epic, "--role", "orchestrator", "--agent", "orch-cli", "--harness", "claude_code", "--harness-session", "o-1");
    ...
    const shown = (await cli("show", epic)).json;
    ...
    const ended = await cli("attempt", "end", epic, "--role", "orchestrator", "--agent", "orch-cli");
    ...
    expect((await cli("attempt", "open", epic, "--agent", "orch-cli")).status).toBe(2);
    expect((await cli("attempt", "open", epic, "--role", "worker", "--agent", "orch-cli")).status).toBe(2);
    const leaf = String((await cli("new", "Leaf")).json.identifier);
    expect((await cli("checkout", leaf, "--role", "orchestrator")).status).toBe(2);
    expect((await cli("status", leaf, "in_progress", "--role", "orchestrator")).status).toBe(2);
...
describe("HTTP /api/action", () => {
  ...
  it("status, checkout and release return `attempt` beside the issue, as the CLI does", async () => {
    ...
    try {
      ...
      const viaHttp = String((await cli("new", "HTTP work")).json.identifier);
      const viaCli = String((await cli("new", "CLI work")).json.identifier);

      const claimed = await post({ type: "checkout", ref: viaHttp });
      const fromCli = (await cli("checkout", viaCli, "--agent", "agent-http")).json;
...
describe("resumeGapSeconds on every read surface", () => {
  ...
  it("show, get_task and /api/issue carry timing.resumeGaps; attempt <id> and get_attempt carry chain[].resumeGapSeconds", async () => {
    const ref = String((await cli("new", "Interrupted and resumed")).json.identifier);
    const first = String(attemptOf((await cli("checkout", ref)).json).id);
    await cli("attempt", "interrupt", ref, "--reason", "provider_limit");
    const second = String(attemptOf((await cli("checkout", ref)).json).id);
    const link = { attemptId: first, resumedByAttemptId: second, resumeGapSeconds: expect.any(Number), clockSkew: false };

    const shown = (await cli("show", ref)).json;
    ...
    const detail = (await cli("attempt", first)).json;
